...
export async function GET(request: NextRequest) {
  ...
  // Use service client to fetch parent agent's phones bypassing RLS
  const db = createServiceClient()

  let query = db
    .from('phones')
    .select('id, brand, model, storage, color, selling_price, down_payment, weekly_payment, payment_weeks, imei, status', { count: 'exact' })
  ...
  if (error) {
    console.error('Failed to fetch agent phones for subagent:', error)
    return NextResponse.json({ error: 'Failed to fetch phones' }, { status: 500 })
  }

  return NextResponse.json({ phones: phones ?? [], total: count ?? 0, page, limit })
}
